...
class CapturedDocument {
    ...
    validateFields(): boolean {
        if (!validator.matches(this.ArticleSourceId, CapturedDocument.ID_PATTERN)
            || !validator.matches(this.DocumentId, CapturedDocument.ID_PATTERN)
            || !validator.matches(this.ArchiveBucket, CapturedDocument.ARCHIVE_BUCKET_PATTERN)) {
            return false;
        }
        return true;
    }
